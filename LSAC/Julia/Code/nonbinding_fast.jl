...
function solve_CPM_subproblem(
    ...
    params1c, params1n, β_p, β_c, φ, Vc_star, τ_star, R_star, M_star, Vp_star, t, τ_p
)

    # Bounds (match JuMP constraints)
    #τ_lower = τ_star + 1e-4                     # enforce τ_p >= τ_star + epsilon
    #τ_upper = Time - s_it - 0.1                # as before
    R_lower = 0.1
    M_lower = 0.1
    ...
    #NLopt.upper_bounds!(opt, [τ_upper, 1e6, 1e6])

    # Set bounds
    NLopt.lower_bounds!(opt, [R_lower, M_lower])
    # keep generous upper bounds for R and M to mimic Ipopt's unboundedness (but finite)
    NLopt.upper_bounds!(opt, [1e6, 1e6])
    ...
    function objective(x, grad)
        ...
        Vp_cpm = ((1-φ)*α_1*log(c_p) + φ*λ_1*log(l_p) + φ*λ_2*log(R_p) - ζ +
                ...
                β_p*ψpn*(params1n[t*4-1]*log(τ_p)))
        
        return Vp_cpm  # NLopt maximizes
    end
    
    # Constraint: Vc_cpm - Vc_star >= 0
    function constr_vc(x, grad)
        #τ_p, R_p, M_p = x
        R_p, M_p = x
        l_p = Time - s_it - τ_p
        # protect domain
        if l_p <= 0.0 || R_p <= 0.0 || τ_p <= 0.0 || M_p <= 0.0
            return -1e20
        end

        Vc_cpm = (λ_1*log(l_p) + λ_2*log(R_p) +
                 β_c*ψcc*(params1c[t*5-2]*log(τ_p) + params1c[t*5-1]*log(M_p)) +
                 β_c*ψcn*(params1n[t*4-1]*log(τ_p)))

        #@show τ_p R_p M_p constraint_val=Vc_cpm - Vc_star
        
        return Vc_cpm - Vc_star  # >= 0
    end

    # Constraint: l_p >= 0.1  -> (l_p - 0.1) >= 0
    ...
    #x0 = [τ_start, max(R_lower, R_star), max(M_lower, M_star)]
...
function value_func(rng, params1c, params1n, params, n_c, n_age, s, Time, Y, CT)
    ...
    Threads.@threads for i = 1:n_c
        ...
        for t = n_age:-1:1
            ...
            else # ψ_{c, 3}^C, ψ_{c, 3}^N, ψ_{p, 3}^C, ψ_{p, 3}^N, ψ_{c, 2}^C, ψ_{c, 2}^N, ψ_{p, 2}^C, ψ_{p, 2}^N
                ψcc_store[i, t] = λ_0 + β_c*(params1c[t*5+2]*ψcc_store[i, t+1] + params1n[t*4+4]*ψcn_store[i, t+1]) # ψ_{c, T}^C
                ψcn_store[i, t] = λ_3 + β_c*(params1n[t*4+2]*ψcn_store[i, t+1] + params1c[t*5+5]*ψcc_store[i, t+1]) # ψ_{c, T}^N
                ψpc_store[i, t] = (1-φ)*α_0 + φ*λ_0 + β_p*(params1c[t*5+2]*ψpc_store[i, t+1] +
                 params1n[t*4+4]*ψpn_store[i, t+1]) # ψ_{p, T + 1}^C
                ψpn_store[i, t] = (1-φ)*α_2 + φ*λ_3 + β_p*(params1n[t*4+2]*ψpn_store[i, t+1] +
                 params1c[t*5+5]*ψpc_store[i, t+1]) # ψ_{p, T + 1}^N
            end
            ...
            χ = (1-φ)*α_1 + φ*λ_2 + β_p*ψpc*params1c[t*5-1]
            c_star = (1-φ)*α_1*(Y_it+CT_it) / χ # Parents' Consumption
            R_star = φ*λ_2*(Y_it+CT_it) / χ # Child's Allowances
            M_star = β_p*ψpc*params1c[t*5-1]*(Y_it+CT_it) / χ # Educational Investment Goods

            C = β_p*(ψpc*params1c[t*5-2] + ψpn*params1n[t*4-1])
            A = β_c*(ψcc*params1c[t*5-2] + ψcn*params1n[t*4-1])
            τ_star = A*(Time-s_it) / (λ_1+A) # Closed-form optimal child effort (no CPM)
            l_star = Time - s_it - τ_star
            Vc_star = λ_1*log(l_star) + λ_2*log(R_star) + β_c*ψcc*(params1c[t*5-2]*log(τ_star) + params1c[t*5-1]*log(M_star)) +
             β_c*ψcn*params1n[t*4-1]*log(τ_star)
            Vp_star = (1-φ)*α_1*log(c_star) + φ*λ_1*log(l_star) + φ*λ_2*log(R_star) +
             β_p*ψpc*(params1c[t*5-2]*log(τ_star) + params1c[t*5-1]*log(M_star)) +
              β_p*ψpn*params1n[t*4-1]*log(τ_star)
